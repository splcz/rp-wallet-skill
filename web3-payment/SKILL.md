...
| 选项 ID | 描述 |
|---------|------|
| `user` | 用户签名 — 打开浏览器，通过 MetaMask 或 WalletConnect 签名 |
| `agent` | Agent 签名 — 使用已存储的私钥自动签名（无需人工操作） |

### 第二步：执行签名

#### 用户选择"用户签名"

调用 `redotpay-usdc` MCP 服务器的 `sign_payment` 工具：

```json
{ "paymentRequiredBase64": "...", "acceptIndex": 0, "signer": "user" }
```
...
先调用 `redotpay-usdc` MCP 服务器的 `get_agent_wallet` 工具检查是否有存储的钱包。

- **如果已有钱包**：直接调用 `sign_payment`：

```json
{ "paymentRequiredBase64": "...", "acceptIndex": 0, "signer": "agent" }
```

- **如果没有钱包**：通过对话让用户提供私钥，然后调用 `redotpay-usdc` MCP 服务器的 `set_agent_wallet` 工具：
...
所有钱包管理工具均通过 `redotpay-usdc` MCP 服务器调用。
...
调用 `get_agent_wallet`（无参数）。

### 删除钱包

调用 `remove_agent_wallet`（无参数）。

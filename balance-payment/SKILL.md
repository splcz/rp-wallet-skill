...
```
CallMcpTool: redotpay-usdc → check_authorization
Arguments: {}
```
...
- **如果返回二维码内容**：设备需要授权。
  ...
  2. **立即**调用 `confirm_authorization`（无需等待用户回复），工具会自动轮询服务端直到用户完成扫码或超时：

```
CallMcpTool: redotpay-usdc → confirm_authorization
Arguments: { "qrCode": "check_authorization 返回的二维码内容" }
```

  3. 若返回授权成功，继续第二步签名。
  4. 若返回超时或已过期，提示用户并重新 `check_authorization` 获取新二维码。

### 第二步：执行签名

调用 `sign_payment` 进行签名（**不传** `signer` 参数，由 MCP 使用已授权的凭证密钥签名）：

```
CallMcpTool: redotpay-usdc → sign_payment
Arguments: { "paymentRequiredBase64": "...", "acceptIndex": 1 }
```
...
调用 `reset_authorization` 清除本地旧密钥与凭证并生成新二维码：

```
CallMcpTool: redotpay-usdc → reset_authorization
Arguments: {}
```
